...
string GetRandomWord(const string& filename) { //функция для генерации рандомного слова
    ...
    if (!file.is_open()) { // Проверка на успешное открытие файла
        ...
        return ""; // Возвращаем пустую строку или можно обработать ошибку иначе
    }

    std::string sentence, word;
    std::vector<std::string> words;

    while (std::getline(file, sentence)) { // Чтение строк из файла
        std::stringstream text(sentence);
        while (text >> word) {
            words.push_back(word); // Добавление слов в вектор
        }
    }

    file.close();

    if (words.empty()) { // Проверка на пустой вектор
